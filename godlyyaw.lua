...
-- ----------------------------------------------------------------------------
-- Per-state anti-aim builder with a necrotool-style navigation menu (Kittyhook /
-- EmberLash style: a state list you drill into). Each command the active state
-- is detected (Stand / Move / Slow walk / Air / Crouch) and its config is pushed
-- into gamesense's own anti-aim engine (pitch, yaw base, yaw, jitter, body yaw,
-- freestanding) for a real jitter + desync + fake-pitch + freestanding AA.
--
-- IMPORTANT: load THIS file (godlyyaw.lua) - a filename with a dot in the middle
-- (godly.yaw.lua) can fail to register in gamesense. The whole init is wrapped in
-- pcall; if anything goes wrong it prints the exact reason to the gamesense
-- console via client.error_log instead of failing silently.
--
-- UI lives under LUA > B. Per-state config persists to the database.
-- ============================================================================
...
local function main()
    ...
    local focused = nil
    local loading = false
    local update_vis            -- forward declaration
    ...
    local function load_focused()
        ...
        ui.set(w_enabled, c.enabled); ui.set(w_pitch, c.pitch); ui.set(w_base, c.base)
        ui.set(w_yaw, c.yaw_add); ui.set(w_style, c.style); ui.set(w_range, c.range)
        ui.set(w_body, c.body); ui.set(w_body_amt, c.body_amt); ui.set(w_free, c.freestand)
        loading = false
    end
    local function save_focused()
        if loading or not focused then return end
        local c = config[focused]
        c.enabled = ui.get(w_enabled); c.pitch = ui.get(w_pitch); c.base = ui.get(w_base)
        c.yaw_add = ui.get(w_yaw); c.style = ui.get(w_style); c.range = ui.get(w_range)
        c.body = ui.get(w_body); c.body_amt = ui.get(w_body_amt); c.freestand = ui.get(w_free)
        persist()
    end
    ...
    update_vis = function()
        ...
        for _, e in ipairs(settings_elems) do ui.set_visible(e, show) end
...
if not ok then
    pcall(client.error_log, "godly.yaw failed to load: " .. tostring(err))
end
