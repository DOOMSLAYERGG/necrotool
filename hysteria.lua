...
-- ============================================================================
-- EmberLash-style anti-aim builder (appended to hysteria, fully self-contained)
-- ----------------------------------------------------------------------------
-- Ported from EmberLash v3: the per-condition builder concept plus the full
-- 20 yaw modes + 20 pitch modes (setup_yaw / setup_pitch). It builds its own
-- menu under the gamesense LUA tab and drives gamesense's built-in anti-aim
-- engine (Yaw base / Yaw / Body yaw / Freestanding / Edge yaw / Pitch), so it
-- runs independently of hysteria's own (obfuscated) menu framework.
--
-- Master toggle defaults OFF so it never interferes with hysteria's own AA
-- until you enable it. Config persists to the database with flat keys. The
-- whole thing lives in a do-block with a single pcall'd main() so it adds no
-- top-level locals to the host chunk and can never break hysteria if it errors.
-- ============================================================================
do
local function main()
    local DB_KEY = "hysteria_emberlash_aa_v1"

    local CONDS = { "Standing", "Moving", "Walking", "Crouching", "In air", "In air-crouch" }

    local YAW_BASES = { "Local view", "At targets", "Away from targets" }
    local BODY_OPTS = { "Off", "Static", "Jitter", "Opposite" }
    local pitch_modes = {
        "STATIC_DOWN", "STATIC_UP", "STATIC_ZERO", "RANDOM", "OSCILLATE", "JITTER", "DISTORT", "SWAY",
        "UNPREDICTABLE", "STAIR", "WAVE_CHAOS", "SNAP_RANDOM", "PULSE", "SPIRAL", "DRIFT", "HEARTBEAT",
        "TRIANGLE", "GLITCH", "PENDULUM", "CHAOTIC_BLEND"
    }
    local yaw_modes = {
        "STATIC_LEFT", "STATIC_RIGHT", "RANDOM", "SWAY", "JITTER", "DISTORT", "OSCILLATE", "STEP",
    ...
    local function default_cfg()
        return {
            ...
            yaw_offset = 0,
            yaw_mode  = "SWAY",
            yaw_angle = 90, yaw_speed = 1.0, yaw_intensity = 1.0,
            pitch_mode = "HEARTBEAT",
            pitch_angle = 45, pitch_speed = 1.0, pitch_intensity = 1.0,
            -- body yaw (Kittyhook-style): Off/Static/Jitter/Opposite, with a
            -- Gamesense(symmetric)/Advanced(separate L/R) source and a modulation
            -- type (Default/Spin/Fluctuate/Random/Dynamic).
            body_yaw  = "Jitter", bd_type = "Gamesense", body_type = "Default",
            body_static = 60, body_left = -60, body_right = 60,
            -- freeze (EmberLash-style): periodically hold the yaw angle so the
            -- anti-aim "freezes" and breaks the predictable jitter pattern.
            freeze = false, freeze_chance = 0, freeze_time = 0, freeze_cooldown = 1,
            freestand = true, edge_yaw = false,
        }
    end

    local config = {}
    for _, c in ipairs(CONDS) do config[c] = default_cfg() end

    -- ---------------------------------------------------------------- persistence
    local function persist()
        local f = {}
        for _, s in ipairs(CONDS) do
            local c = config[s]
            f[s.."|en"], f[s.."|yb"], f[s.."|yo"] = c.enabled, c.yaw_base, c.yaw_offset
            f[s.."|ym"], f[s.."|ya"], f[s.."|ys"] = c.yaw_mode, c.yaw_angle, c.yaw_speed
            f[s.."|yi"], f[s.."|pm"], f[s.."|pa"] = c.yaw_intensity, c.pitch_mode, c.pitch_angle
            f[s.."|ps"], f[s.."|pi"], f[s.."|bo"] = c.pitch_speed, c.pitch_intensity, c.body_yaw
            f[s.."|bt"], f[s.."|by"], f[s.."|bs"] = c.bd_type, c.body_type, c.body_static
            f[s.."|bl"], f[s.."|br"]              = c.body_left, c.body_right
            f[s.."|fz"], f[s.."|fc"], f[s.."|ft"] = c.freeze, c.freeze_chance, c.freeze_time
            f[s.."|fd"], f[s.."|fs"], f[s.."|ed"] = c.freeze_cooldown, c.freestand, c.edge_yaw
        end
        pcall(database.write, DB_KEY, f)
    end
    local function restore()
        ...
        for _, s in ipairs(CONDS) do
            ...
            if f[s.."|yb"] then c.yaw_base = f[s.."|yb"] end
            if f[s.."|yo"] then c.yaw_offset = tonumber(f[s.."|yo"]) or c.yaw_offset end
            if f[s.."|ym"] then c.yaw_mode = f[s.."|ym"] end
            if f[s.."|ya"] then c.yaw_angle = tonumber(f[s.."|ya"]) or c.yaw_angle end
            if f[s.."|ys"] then c.yaw_speed = tonumber(f[s.."|ys"]) or c.yaw_speed end
            if f[s.."|yi"] then c.yaw_intensity = tonumber(f[s.."|yi"]) or c.yaw_intensity end
            if f[s.."|pm"] then c.pitch_mode = f[s.."|pm"] end
            if f[s.."|pa"] then c.pitch_angle = tonumber(f[s.."|pa"]) or c.pitch_angle end
            if f[s.."|ps"] then c.pitch_speed = tonumber(f[s.."|ps"]) or c.pitch_speed end
            if f[s.."|pi"] then c.pitch_intensity = tonumber(f[s.."|pi"]) or c.pitch_intensity end
            if f[s.."|bo"] then c.body_yaw = f[s.."|bo"] end
            if f[s.."|bt"] then c.bd_type = f[s.."|bt"] end
            if f[s.."|by"] then c.body_type = f[s.."|by"] end
            if f[s.."|bs"] then c.body_static = tonumber(f[s.."|bs"]) or c.body_static end
            if f[s.."|bl"] then c.body_left = tonumber(f[s.."|bl"]) or c.body_left end
            if f[s.."|br"] then c.body_right = tonumber(f[s.."|br"]) or c.body_right end
            if f[s.."|fz"] ~= nil then c.freeze = f[s.."|fz"] end
            if f[s.."|fc"] then c.freeze_chance = tonumber(f[s.."|fc"]) or c.freeze_chance end
            if f[s.."|ft"] then c.freeze_time = tonumber(f[s.."|ft"]) or c.freeze_time end
            if f[s.."|fd"] then c.freeze_cooldown = tonumber(f[s.."|fd"]) or c.freeze_cooldown end
            if f[s.."|fs"] ~= nil then c.freestand = f[s.."|fs"] end
            if f[s.."|ed"] ~= nil then c.edge_yaw = f[s.."|ed"] end
        end
    end
    restore()

    -- ---------------------------------------------------------------- mode math
    -- faithful ports of EmberLash's setup_pitch / setup_yaw (20 modes each).
    local rf = client.random_float
    local function idx_of(list, name)
    ...
    local function setup_pitch(y)
        ...
        local f = idx_of(pitch_modes, y.mode)
        local _, u, M, o = y.speed or 1, y.angle or 45, y.max_angle or 89, y.intensity or 1
        if f == 1 then
            local a = math.min(M, 89); return math.max(-89.0, math.min(a + rf(-2.0, 2) * o, 89))
        elseif f == 2 then
            local a = -math.min(M, 89); return math.max(-89.0, math.min(a + rf(-2.0, 2) * o, 89))
        elseif f == 3 then
            return math.max(-5.0, math.min(math.cos(P * 0.005 * _) * 3 * o, 5))
        elseif f == 4 then
            local a = math.cos(P * 0.02); local X = M * (0.7 + 0.3 * o); return rf(-X, X) + a * 10
        elseif f == 5 then
            local a = 0.03 * _ * (1 + math.cos(P * 0.01) * 0.3); local X = u * (0.8 + 0.2 * math.cos(P * 0.007))
            return math.max(-M, math.min(X * math.cos(P * a), M))
        elseif f == 6 then
            local a = ((P % 8) < 3) and 1.5 or 0.7; local X = u * a * o; return rf(-X, X)
        elseif f == 7 then
            local a = (16 / _) + rf(-3.0, 3); local X = P % a
            if X < a * 0.3 then return math.min(M, u) + rf(-8.0, 8)
            elseif X < a * 0.6 then return -math.min(M, u) + rf(-8.0, 8)
            else return rf(-u * 0.5, u * 0.5) end
        elseif f == 8 then
            local a = u * math.cos(P * 0.08 * _); local X = u * 0.3 * math.cos(P * 0.023 * _ + math.pi / 4)
            local b = u * 0.15 * math.cos(P * 0.041); return math.max(-M, math.min(a + X + b, M))
        elseif f == 9 then
            local a = u * math.cos(P * 0.025 * _ + rf(-8.0, 8)); local X = rf(-18.0, 18) * o
            local b = ((P % 17) == 0) and rf(-30.0, 30) or 0; return math.max(-M, math.min(a + X + b, M))
        elseif f == 10 then
            local a = math.floor(30 / _) + rf(-5.0, 5); local X = (P / a) % 5
            local tbl = { -u, -u * 0.5, 0, u * 0.5, u }; local b = tbl[math.floor(X) + 1] or 0
            return math.max(-M, math.min(b + rf(-3.0, 3), M))
        elseif f == 11 then
            local a = u * 0.5 * math.cos(P * 0.032 * _); local X = u * 0.3 * math.cos(P * 0.051 * _ + 1.2)
            local b = u * 0.2 * math.cos(P * 0.077 * _ + 2.4); local t = rf(-5.0, 5) * o
            return math.max(-M, math.min(a + X + b + t, M))
        elseif f == 12 then
            local a = math.floor(12 / _); if a < 1 then a = 1 end
            if (P % a) == 0 then local tbl = { -M, -u, 0, u, M }; return tbl[math.floor(rf(1, #tbl + 0.999))] or 0 end
            return 0
        elseif f == 13 then
            local a = 4 * _; local X = (P * a) % 100
            if X < 15 then return M elseif X < 30 then return -M elseif X < 45 then return u * 0.5 else return -u * 0.3 end
        elseif f == 14 then
            local a = 1 + ((P * 0.0001 * _) % 3); local X = 0.025 * _ * a; local v2 = u * (1 + math.cos(P * 0.008) * 0.4)
            return math.max(-M, math.min(v2 * math.cos(P * X), M))
        elseif f == 15 then
            local a = (P / 8) % 360; local X = math.cos(a * 0.1) * u; return math.max(-M, math.min(X + rf(-8.0, 8) * o, M))
        elseif f == 16 then
            local d = math.floor(80 / _); if d < 1 then d = 1 end; local a = P % d
            if a < 8 then return u * 0.8 elseif a < 16 then return -u * 0.4 elseif a < 24 then return u * 0.6 else return 0 end
        elseif f == 17 then
            local a = 60 / _; local X = (P % a) / a; local r
            if X < 0.5 then r = u * (X * 4 - 1) else r = u * (3 - X * 4) end
            return math.max(-M, math.min(r, M))
        elseif f == 18 then
            if rf(1, 100) < 15 * o then return rf(-M, M) end
            local a = u * math.cos(P * 0.04 * _); if (P % 7) == 0 then return -a end
            return math.max(-M, math.min(a, M))
        elseif f == 19 then
            local a = 50 / _; local X = (u / 89) * math.pi * 0.5
            local b = u * math.cos(math.sqrt(0.98 / a) * P * 0.1 * _ + X); local d = 1 - ((P % 500) * 0.001)
            return math.max(-M, math.min(b * d, M))
        elseif f == 20 then
            local a = P * 0.01 * _; local v1 = u * math.cos(a * 3.14159); local v2 = u * 0.6 * math.cos(a * 1.618)
            local X = u * 0.4 * math.sin(a * 2.718); local d = rf(-12.0, 12) * o
            local e = ((P % 23) == 0) and rf(-25.0, 25) or 0; return math.max(-M, math.min(v1 + v2 + X + d + e, M))
        end
        return 0
    end
    ...
    local function setup_yaw(y)
        local P = globals.tickcount()
        local f = idx_of(yaw_modes, y.mode)
        local K, u, M, o = y.speed or 1, y.angle or 90, y.max_angle or 180, y.intensity or 1
        if f == 1 then
            return math.max(-180.0, math.min(-math.min(u, M) + rf(-3.0, 3) * o, 180))
        elseif f == 2 then
            return math.max(-180.0, math.min(math.min(u, M) + rf(-3.0, 3) * o, 180))
        elseif f == 3 then
            local a = ((P / 30) % 3) - 1; return a * M * 0.6 + rf(-M * 0.4, M * 0.4)
        elseif f == 4 then
            local a = u * (0.7 + 0.3 * math.cos(P * 0.011)); local X = 0.04 * K * (1 + math.sin(P * 0.007) * 0.2)
            return math.max(-M, math.min(a * math.cos(P * X), M))
        elseif f == 5 then
            local a = ((P % 12) < 4) and 1.6 or 0.8; local X = u * a * o; return rf(-X, X)
        elseif f == 6 then
            local a = (22 / K) + rf(-4.0, 4); local X = P % a
            if X < a * 0.25 then return math.min(u, M) + rf(-12.0, 12)
            elseif X < a * 0.5 then return -math.min(u, M) + rf(-12.0, 12)
            elseif X < a * 0.75 then return rf(-u * 0.7, u * 0.7)
            else return math.cos(P * 0.15) * u * 0.5 end
        elseif f == 7 then
            local a = u * math.cos(P * 0.018 * K); local X = u * 0.3 * math.cos(P * 0.037 * K + 0.5)
            return math.max(-M, math.min(a + X, M))
        elseif f == 8 then
            local a = 7 + ((P / 100) % 3); local X = math.floor(14 / K) + rf(-2.0, 2); if X < 1 then X = 1 end
            local b = (2 * M) / a; local t = (P / X) % a; return -M + (t * b) + rf(-5.0, 5)
        elseif f == 9 then
            local a = math.floor((P / 20) % 4)
            if a == 0 then return rf(-M, M)
            elseif a == 1 then return u * math.cos(P * 0.045 * K)
            elseif a == 2 then return rf(-u, u) + math.sin(P * 0.03) * u * 0.3
            else local b = ((P % 3) == 0) and rf(-40.0, 40) or 0; return b + math.cos(P * 0.02) * u * 0.5 end
        elseif f == 10 then
            local a = math.floor(48 / K); if a < 1 then a = 1 end; local X = P % a
            if X < 10 then return -M + (X / 10) * 2 * M
            elseif X < (a / 2 - 10) then return M
            elseif X < (a / 2 + 10) then return M - (((X - a / 2) + 10) / 10) * 2 * M
            elseif X < (a - 10) then return -M
            else return -M + (((X - a) + 10) / 10) * 2 * M end
        elseif f == 11 then
            local a = u * 0.6 * math.cos(P * 0.028 * K); local X = u * 0.4 * math.cos(P * 0.047 * K + 0.8)
            local b = u * 0.25 * math.cos(P * 0.063 * K + 1.6); local t = rf(-8.0, 8) * o
            return math.max(-M, math.min(a + X + b + t, M))
        elseif f == 12 then
            local a = 1 + (P * 0.0002 * K); local X = ((P * a * K * 0.5) % 360) - 180; return math.max(-M, math.min(X, M))
        elseif f == 13 then
            local a = u * math.cos(P * 0.035 * K); local X = rf(-8.0, 8) * o
            if (P % 5) == 0 then return a + X * 2 end; return a + X
        elseif f == 14 then
            local angles = { -M, -u, 0, u, M }; local interval = math.max(math.floor(25 / K), 1)
            local i = math.floor(P / interval) % #angles + 1; local phase = (P % interval) / interval
            return angles[i] + math.cos(phase * math.pi * 2) * 10 * o
        elseif f == 15 then
            local a = 1 + ((P % 200) * 0.005); local b = P * 0.05 * K; local X = math.min(M, u * a); return X * math.cos(b)
        elseif f == 16 then
            local a = math.floor(18 / K); if a < 1 then a = 1 end; if (P % a) == 0 then return rf(-M, M) end; return 0
        elseif f == 17 then
            local a = 70 / K; local b = (P % a) / a; return -M + b * 2 * M
        elseif f == 18 then
            local a = 0.08 * K; local b = u * math.cos(P * a); local d = math.pow(0.998, P % 200)
            return math.max(-M, math.min(b * d, M))
        elseif f == 19 then
            local a = u * 0.7 * math.cos(P * 0.031 * K); local b = u * 0.5 * math.cos(P * 0.019 * K + math.pi / 3)
            return math.max(-M, math.min(a + b, M))
        elseif f == 20 then
            local a = math.floor((P / 50) % 5)
            if a == 0 then return u * math.cos(P * 0.04 * K) + rf(-15.0, 15)
            elseif a == 1 then return rf(-M, M)
            elseif a == 2 then return u * math.sin(P * 0.055 * K) * math.cos(P * 0.023)
            elseif a == 3 then local K2 = (P / 8) % 7; return -M + K2 * ((2 * M) / 6)
            else return u * math.cos(P * 0.03) + u * 0.4 * math.sin(P * 0.017) end
        end
        return 0
    end
    ...
    local master   = ui.new_checkbox("LUA", "B", "EmberLash AA builder")
    local nav_open = ui.new_checkbox("LUA", "B", "eb_nav_open"); ui.set_visible(nav_open, false)
    local nav_lbl  = ui.new_label("LUA", "B", "\aB9BEFFFF- \aFFFFFFFFconditions \aB9BEFFFF-")
    ...
    local w_en   = ui.new_checkbox("LUA", "B", "\aFFFFFFFF  Enabled (this condition)")
    local w_yb   = ui.new_combobox("LUA", "B", "\aFFFFFFFF  Yaw base", YAW_BASES)
    local w_yo   = ui.new_slider("LUA", "B", "\aFFFFFFFF  Yaw offset", -180, 180, 0)
    local w_ym   = ui.new_combobox("LUA", "B", "\aFFFFFFFF  Yaw mode", yaw_modes)
    local w_ya   = ui.new_slider("LUA", "B", "\aFFFFFFFF  Yaw angle", 0, 180, 90)
    local w_ys   = ui.new_slider("LUA", "B", "\aFFFFFFFF  Yaw speed", 1, 30, 10)
    local w_yi   = ui.new_slider("LUA", "B", "\aFFFFFFFF  Yaw intensity", 1, 30, 10)
    local w_pm   = ui.new_combobox("LUA", "B", "\aFFFFFFFF  Pitch mode", pitch_modes)
    local w_pa   = ui.new_slider("LUA", "B", "\aFFFFFFFF  Pitch angle", 0, 89, 45)
    local w_ps   = ui.new_slider("LUA", "B", "\aFFFFFFFF  Pitch speed", 1, 30, 10)
    local w_pi   = ui.new_slider("LUA", "B", "\aFFFFFFFF  Pitch intensity", 1, 30, 10)
    local w_body  = ui.new_combobox("LUA", "B", "\aFFFFFFFF  Body yaw (desync)", BODY_OPTS)
    local w_bdt   = ui.new_combobox("LUA", "B", "\aFFFFFFFF  Body source", { "Gamesense", "Advanced" })
    local w_bty   = ui.new_combobox("LUA", "B", "\aFFFFFFFF  Body type", { "Default", "Spin", "Fluctuate", "Random", "Dynamic" })
    local w_bs    = ui.new_slider("LUA", "B", "\aFFFFFFFF  Body amount", -180, 180, 60)
    local w_bl    = ui.new_slider("LUA", "B", "\aFFFFFFFF  Body left", -180, 180, -60)
    local w_br    = ui.new_slider("LUA", "B", "\aFFFFFFFF  Body right", -180, 180, 60)
    local w_fz    = ui.new_checkbox("LUA", "B", "\aFFFFFFFF  Freeze anti-aim")
    local w_fc    = ui.new_slider("LUA", "B", "\aFFFFFFFF  Freeze chance", 0, 100, 0, true, "%")
    local w_ft    = ui.new_slider("LUA", "B", "\aFFFFFFFF  Freeze time", 0, 100, 0, true, "t")
    local w_fd    = ui.new_slider("LUA", "B", "\aFFFFFFFF  Freeze cooldown", 1, 1000, 1, true, "t")
    local w_free  = ui.new_checkbox("LUA", "B", "\aFFFFFFFF  Freestanding")
    local w_edge  = ui.new_checkbox("LUA", "B", "\aFFFFFFFF  Edge yaw")
    local w_ind   = ui.new_checkbox("LUA", "B", "\aFFFFFFFF  Indicator")

    local elems = { w_en, w_yb, w_yo, w_ym, w_ya, w_ys, w_yi, w_pm, w_pa, w_ps, w_pi,
        w_body, w_bdt, w_bty, w_bs, w_bl, w_br, w_fz, w_fc, w_ft, w_fd, w_free, w_edge }

    local function load_focused()
        if not focused then return end
        loading = true
        local c = config[focused]
        ui.set(w_en, c.enabled); ui.set(w_yb, c.yaw_base); ui.set(w_yo, c.yaw_offset)
        ui.set(w_ym, c.yaw_mode); ui.set(w_ya, c.yaw_angle)
        ui.set(w_ys, math.floor(c.yaw_speed * 10 + 0.5)); ui.set(w_yi, math.floor(c.yaw_intensity * 10 + 0.5))
        ui.set(w_pm, c.pitch_mode); ui.set(w_pa, c.pitch_angle)
        ui.set(w_ps, math.floor(c.pitch_speed * 10 + 0.5)); ui.set(w_pi, math.floor(c.pitch_intensity * 10 + 0.5))
        ui.set(w_body, c.body_yaw); ui.set(w_bdt, c.bd_type); ui.set(w_bty, c.body_type)
        ui.set(w_bs, c.body_static); ui.set(w_bl, c.body_left); ui.set(w_br, c.body_right)
        ui.set(w_fz, c.freeze); ui.set(w_fc, c.freeze_chance)
        ui.set(w_ft, c.freeze_time); ui.set(w_fd, c.freeze_cooldown)
        ui.set(w_free, c.freestand); ui.set(w_edge, c.edge_yaw)
        loading = false
    end
    local function save_focused()
        if loading or not focused then return end
        local c = config[focused]
        c.enabled = ui.get(w_en); c.yaw_base = ui.get(w_yb); c.yaw_offset = ui.get(w_yo)
        c.yaw_mode = ui.get(w_ym); c.yaw_angle = ui.get(w_ya)
        c.yaw_speed = ui.get(w_ys) / 10; c.yaw_intensity = ui.get(w_yi) / 10
        c.pitch_mode = ui.get(w_pm); c.pitch_angle = ui.get(w_pa)
        c.pitch_speed = ui.get(w_ps) / 10; c.pitch_intensity = ui.get(w_pi) / 10
        c.body_yaw = ui.get(w_body); c.bd_type = ui.get(w_bdt); c.body_type = ui.get(w_bty)
        c.body_static = ui.get(w_bs); c.body_left = ui.get(w_bl); c.body_right = ui.get(w_br)
        c.freeze = ui.get(w_fz); c.freeze_chance = ui.get(w_fc)
        c.freeze_time = ui.get(w_ft); c.freeze_cooldown = ui.get(w_fd)
        c.freestand = ui.get(w_free); c.edge_yaw = ui.get(w_edge)
        persist()
    end

    update_vis = function()
        local on, open = ui.get(master), ui.get(nav_open)
        ui.set_visible(nav_lbl, on and not open)
        ...
        for _, e in ipairs(elems) do ui.set_visible(e, show) end
        ui.set_visible(w_ind, on and not open)
    end

    ui.set_callback(master, update_vis)
    for _, e in ipairs(elems) do ui.set_callback(e, save_focused) end
    update_vis()
    ...
    local r_jit_mode              = ref2("AA", "Anti-aimbot angles", "Yaw jitter")
    ...
    local function setref(ref, ...) if ref then pcall(ui.set, ref, ...) end end
    ...
    local function detect(me)
        ...
        if speed < 135 then return "Walking" end
        return "Moving"
    end

    local hud = { cond = "", yaw = 0, frozen = false }

    -- EmberLash-style freeze runtime (single active condition at a time)
    local fz = { frozen = false, until_tick = 0, last_tick = 0, held = 0 }

    local function compute_body(c, me, to_jitter)
        local bl, br
        if c.bd_type == "Gamesense" then bl, br = -c.body_static, c.body_static
        else bl, br = c.body_left, c.body_right end
        local now = globals.tickcount()
        local bt = c.body_type
        if bt == "Spin" then
            return math.abs(math.sin(globals.curtime() * 5)) * (to_jitter and bl or br)
        elseif bt == "Fluctuate" then
            local fl = 0.5 + 0.5 * math.sin(now * 0.1)
            return (to_jitter and bl or br) * fl
        elseif bt == "Random" then
            local lo, hi
            if to_jitter then lo, hi = math.min(bl, 0), math.max(bl, 0)
            else lo, hi = math.min(0, br), math.max(0, br) end
            lo, hi = math.floor(lo), math.floor(hi)
            if lo >= hi then return lo end
            return math.random(lo, hi)
        elseif bt == "Dynamic" then
            local vx, vy = entity.get_prop(me, "m_vecVelocity")
            local speed = vx and math.sqrt(vx * vx + vy * vy) or 0
            local factor = math.min(speed / 300, 1)
            return (to_jitter and bl or br) * (1 - factor)
        end
        return to_jitter and bl or br   -- Default
    end

    local function apply(c, me)
        setref(r_en, true)
        setref(r_base, c.yaw_base)

        local yaw = norm(c.yaw_offset + setup_yaw({
            mode = c.yaw_mode, speed = c.yaw_speed, angle = c.yaw_angle, max_angle = 180, intensity = c.yaw_intensity }))

        -- freeze: periodically hold the yaw so the anti-aim stops jittering,
        -- breaking the predictable pattern (ported from EmberLash's freeze).
        local now = globals.tickcount()
        if c.freeze and c.freeze_chance > 0 and c.freeze_time > 0 then
            if fz.frozen then
                if now >= fz.until_tick then fz.frozen = false else yaw = fz.held end
            end
            if not fz.frozen and (now - fz.last_tick) >= c.freeze_cooldown then
                if math.random(1, 10000) <= c.freeze_chance * 10 then
                    fz.frozen = true; fz.until_tick = now + c.freeze_time
                    fz.last_tick = now; fz.held = yaw
                end
            end
        else
            fz.frozen = false
        end
        hud.frozen = fz.frozen

        setref(r_yaw_mode, "Static")
        setref(r_yaw_val, yaw)
        setref(r_jit_mode, "Off")            -- we compute the jitter ourselves
        hud.yaw = yaw

        if c.body_yaw == "Off" then
            setref(r_body_mode, "Off")
        else
            -- Jitter alternates the desync side each tick; the others hold a side.
            local to_jitter = (c.body_yaw == "Jitter") and (now % 2 == 0) or false
            local bval = compute_body(c, me, to_jitter)
            if c.body_yaw == "Jitter" then
                -- drive gamesense as Static with our own per-tick value
                setref(r_body_mode, "Static")
            else
                setref(r_body_mode, c.body_yaw)
            end
            setref(r_body_val, bval)
        end

        setref(r_free, c.freestand)
        setref(r_edge, c.edge_yaw)

        -- gamesense's Pitch reference is a combobox, so map the computed pitch
        -- (full 20-mode math) onto its Up/Down/Minimal/Default options each tick.
        local p = setup_pitch({
            mode = c.pitch_mode, speed = c.pitch_speed, angle = c.pitch_angle, max_angle = 89, intensity = c.pitch_intensity })
        if p >= 45 then setref(r_pitch, "Down")
        elseif p <= -45 then setref(r_pitch, "Up")
        elseif math.abs(p) < 5 then setref(r_pitch, "Default")
        else setref(r_pitch, "Minimal") end
    end
    ...
    local function on_setup_command()
        ...
        local cond = detect(me)
        hud.cond = cond
        local c = config[cond]
        if c and c.enabled then apply(c, me) end
    end

    local function on_paint_ui()
        if not ui.get(master) or not ui.get(w_ind) then return end
        local me = entity.get_local_player()
        if not me or not entity.is_alive(me) then return end
        local sx, sy = client.screen_size()
        renderer.text(sx / 2, sy * 0.62, 180, 190, 255, 255, "c", 0,
            string.format("AA builder  [%s]  yaw %.0f%s", hud.cond, hud.yaw,
                hud.frozen and "  \aFF5555FFFROZEN" or ""))
    end
...
if not ok then
    pcall(client.error_log, "hysteria AA builder failed to load: " .. tostring(err))
end
end
