...
local function update_visibility_misc()
    ...
    -- per-feature "Setup" list: one feature focused at a time (or "None").
    -- Each feature shows a "Setup <name>" row that is grey when off and accent
    -- when on; selecting one reveals only that feature's settings.
    local focus = ui.get(UI.setup_focus)
    local none  = (focus == "None")
...
do
    ...
    -- Surface_PlaySound is a no-op when called synchronously at script load and
    -- never fires in the main menu on 'paint'. Mark the file ready, then play it
    -- on the first paint_ui frame (paint_ui fires in the menu too).
    local ready = false
    local played = false
...
local function on_paint()
    ...
    -- Menu backdrop (always on): when the cheat menu is open, dim the game
    -- behind it using the same shade/strength as the intro splash (5,3,10 @220).
    -- Drawn BEFORE every lua overlay/image below, so the watermark, render
    -- image, hitmarker, notifications etc. stay bright on top; the native
    -- gamesense menu (drawn above all paint_ui) also stays bright.
    if ui.is_menu_open() then
        local sx, sy = client.screen_size()
        renderer.rectangle(0, 0, sx, sy, 5, 3, 10, 220)
    end
...
-- paint_ui (not paint) so the intro, watermark, keybinds and other overlays
